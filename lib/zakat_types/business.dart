...
class _businessState extends State<business> {
  ...
  void retrieveIncome (){
    double total = 0.0;
    Firestore.instance.collection("zakatTracker").where('type', isEqualTo: 'Income').where('category', isEqualTo: 'Business').where('userID', isEqualTo: userID).getDocuments().then((querySnapshot) {
  ...
  void retrieveExpense (){
    double total = 0.0;
    Firestore.instance.collection("zakatTracker").where('type', isEqualTo: 'Expense').where('category', isEqualTo: 'Business').where('userID', isEqualTo: userID).getDocuments().then((querySnapshot) {
      querySnapshot.documents.forEach((result) {
        print(result.documentID);
